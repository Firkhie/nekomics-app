<template>
  <template v-if="isLoading">
    <div
      class="flex flex-row w-full h-[125px] md:min-h-[220px] bg-white/10 animate-pulse p-2 md:p-5 gap-3 md:gap-4"
      v-for="index in 4"
      :key="index"
    >
    ...
    <div class="flex-1 flex flex-col justify-between text-justify overflow-hidden">
      <div class="w-full flex flex-col gap-[3px] md:gap-2">
        ...
        <div class="flex flex-row gap-2">
          <span
            class="px-2 bg-white/20 text-xs md:text-sm rounded-md"
            v-for="genre in limitedGenres(comic)" :key="genre"
            >{{ genre }}</span
          >
        </div>
