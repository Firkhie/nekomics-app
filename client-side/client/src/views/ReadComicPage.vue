  ...
  <main class="relative w-full px-3 md:px-8 lg:px-32 mt-5 lg:mt-12 text-white flex-grow">
    <section class="relative w-full flex flex-col md:text-base text-sm">
      <div class="flex flex-col justify-center items-center">
        ...
        <p class="text-center">All chapters are in <router-link to="/detail" class="text-[#FC304B] hover:text-[#E32B43] cursor-pointer">{{ comicName }}</router-link></p>
      ...
      <div class="flex flex-row justify-between items-center mb-4 md:mb-10">
        <div id="chapter-btn" class="relative w-[150px] cursor-pointer group text-xs md:text-sm" @click="toggleChapter">
          ...
          <ul id="chapter-menu" class="absolute flex-col bg-[#1E1F1F] rounded-sm w-full cursor-pointer scroll-container z-30 top-[38px] max-h-[165px] overflow-auto left-0 hidden group-open:block">
            <li class="py-2 px-4 hover:bg-white/5" v-for="chapter in detailComic.detailChapters" :key="chapter.chapterId">Chapter {{ chapter.chapter }}</li>
          </ul>
        </div>
        <div class="flex gap-2">
          <button class="py-2 px-4 md:px-5 bg-[#FC304B] hover:bg-[#E32B43] text-xs md:text-sm rounded-sm">Prev</button>
          <button class="py-2 px-4 md:px-5 bg-[#FC304B] hover:bg-[#E32B43] text-xs md:text-sm rounded-sm">Next</button>
        </div>
      </div>
      <div v-if="isLoading" class="flex flex-col justify-between items-center w-max-[700px]">
        <div id="container">
          <svg viewBox="0 0 100 100">
            ...
            <circle id="spinner" style="fill:transparent;stroke:#FC304B;stroke-width: 7px;stroke-linecap: round;filter:url(#shadow);" cx="50" cy="50" r="45"/>
          </svg>
        </div>
      </div>
