<template>
  <main
    v-if="isLoading"
    class="relative w-full max-w-screen-xl mx-auto px-3 md:px-8 text-white flex-grow animate-pulse"
  >
    <section class="w-full flex items-center md:items-start flex-col md:flex-row gap-3 lg:gap-5">
      <!-- LEFT -->
      <div
        class="flex flex-col items-center md:items-start justify-center md:justify-start gap-3 w-full md:max-w-[240px] lg:max-w-[300px] h-auto md:min-h-[600px]"
      >
      ...
      <div class="flex-1 flex flex-col text-justify gap-3 w-full">
        <div
          class="flex flex-col justify-between p-4 bg-white/10 min-h-[200px] md:min-h-[350px] rounded-sm"
        >
  ...
  <main v-else class="relative w-full max-w-screen-xl px-6 md:px-8 mx-auto mt-5 text-white flex-grow">
    ...
    >
      <!-- LEFT -->
      <div
        class="flex flex-col items-center md:items-start justify-center md:justify-start gap-3 w-full md:max-w-[240px] lg:max-w-[300px] h-auto md:min-h-[850px] lg:text-base md:text-sm text-xs"
      >
        <div class="max-w-[400px] md:w-auto">
          <img
            v-bind:src="detailComic.coverArt"
            alt=""
            class="w-auto h-auto rounded-sm bg-center bg-cover"
          />
        </div>
        <button
          class="p-3 bg-[#4252FF] hover:bg-[#3C4BEC] w-full rounded-sm flex justify-center items-center gap-3"
          @click.prevent="submitCreateBookmark(detailComic.id)"
        >
      ...
      <div class="flex-1 flex flex-col text-justify gap-3">
        <div
          class="flex flex-col justify-between p-4 bg-[#1E1F1F] min-h-[320px] md:min-h-[350px] rounded-sm"
        >
          <div class="flex flex-col gap-2 md:gap-3">
            <h4 class="line-clamp-2">{{ detailComic.title }}</h4>
            <div class="flex flex-wrap gap-2">
              <span
                class="px-4 bg-white/30 text-xs md:text-sm rounded-lg"
                v-for="genre in limitGenres"
                :key="genre"
                >{{ genre }}</span
              >
            </div>
          ...
          <div class="flex gap-3">
            <button
              ...
              @click="
                submitChapterPagesId(
                  detailComic.id,
                  detailComic.title,
                  detailComic.detailChapters[detailComic.detailChapters.length - 1].chapterId,
                  detailComic.detailChapters[detailComic.detailChapters.length - 1].chapter
                )
              "
            >
              First Chapter
            ...
            <button
              ...
              @click="
                submitChapterPagesId(
                  detailComic.id,
                  detailComic.title,
                  detailComic.detailChapters[0].chapterId,
                  detailComic.detailChapters[0].chapter
                )
              "
            >
              Latest Chapter
